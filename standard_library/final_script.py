...
text_bits = encoder(np.array([int(b) for b in uncoded_text_bits]))

print(f"Length of bits before encoding = {l_before}")
print(f"Length of bits after encoding = {len(text_bits)}")
print(f"Rate = {l_before/len(text_bits)}")
...
if sys.argv[1] == 'send':
    ...
    transmitter.full_pipeline(sync, text_bits, f"full_pipeline_transmission_audio{reader_idx}")
    ...
    video_folder_name = "videos_linreg"
    ...
    for nw in [0.0]:#, 0.1, 0.15, 0.2, 0.25]:
        ...
        error_rate = 1-sum(text_bits[i] == received_bitstring[i] for i in range(len(uncoded_text_bits)))/len(uncoded_text_bits)        
        print(nw, error_rate)
        if not nw:
            pass
