...
sync = Synchronisation(["chirp", "pilot"], pilot_idx=np.arange(0, N/2, 32), pilot_symbol=-1 - 1j,chirp_length=T, chirp_func=c_func, N=N, L=L, num_OFDM_symbols_chirp=35)
# sync = Synchronisation(["chirp"], chirp_length=T, chirp_func=c_func, N=N, L=L, num_OFDM_symbols_chirp=35)

if sys.argv[1] == 'send':
    transmitter = Transmitter("gray", N=N, L=L, num_estimation_symbols=num_estimation_symbol, bits_filename="full_pipeline_random_bits", synchronisation=sync)
    transmitter.full_pipeline(sync, text_bits, "full_pipeline_transmission_audio")
    ...
    recording_file = "final_pipeline_message_with_estimation"
    ...
    #channel_input = np.load("full_pipeline_transmission_audio.npy").reshape(-1)
    #channel_output = channel.transmit(channel_input)
