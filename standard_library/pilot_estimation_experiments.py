...
sync_dict = {
    "no_pilot": Synchronisation(["chirp"], chirp_length=T, chirp_func=c_func, N=N, L=L, num_OFDM_symbols_chirp=35),
    "pilot_32": Synchronisation(["chirp", "pilot"], pilot_idx=np.arange(0, N/2, 32), pilot_symbol=-1 - 1j,chirp_length=T, chirp_func=c_func, N=N, L=L, num_OFDM_symbols_chirp=35),
    "pilot_16": Synchronisation(["chirp", "pilot"], pilot_idx=np.arange(0, N/2, 16), pilot_symbol=-1 - 1j,chirp_length=T, chirp_func=c_func, N=N, L=L, num_OFDM_symbols_chirp=35),
    "pilot_8": Synchronisation(["chirp", "pilot"], pilot_idx=np.arange(0, N/2, 8), pilot_symbol=-1 - 1j,chirp_length=T, chirp_func=c_func, N=N, L=L, num_OFDM_symbols_chirp=35),
}

new_weights = [0, 0.01, 0.05, 0.1]
noise_vars = [0, 0.0001, 0.001, 0.01, 0.1]
artificial_channel_impulse = np.array(pd.read_csv("channel.csv", header=None)[0])

receiver = Receiver(N=N, L=L, constellation_name="gray", num_estimation_symbols=num_estimation_symbol, num_message_symbols=35)
channel = Channel(artificial_channel_impulse)
modu = Modulation("gray", N, L)

print("preparing audio files")
for name, sync in tqdm(sync_dict.items()):
    fname = f"pilot_experiments/audio_{name}"
    transmitter = Transmitter("gray", N=N, L=L, num_estimation_symbols=num_estimation_symbol, bits_filename=f"pilot_experiments/random_bits_{name}", synchronisation=sync)
    transmitter.full_pipeline(sync, text_bits, fname)

with open("pilot_experiments/results.txt", "w") as results_file:
    results_file.write("PROT\tNVAR\tNWEI\tSCORE\n")

    print("collecting results data")
    for name, sync in tqdm(sync_dict.items()):
        channel_input = np.load(f"pilot_experiments/audio_{name}.npy").reshape(-1)

        for noise_var in noise_vars:
            channel_output = channel.transmit(channel_input, noise=noise_var)

            for new_weight in new_weights:
                ground_truth_estimation_OFDM_frames = get_OFDM_data_from_bits(modu, sync, source_bits = f"pilot_experiments/random_bits_{name}")
                received_bitstring = receiver.full_pipeline(
                    channel_output, sync, ground_truth_estimation_OFDM_frames, sample_shift = 0, new_weight = new_weight
                )

                score = sum(text_bits[i] == received_bitstring[i] for i in range(len(text_bits)))/len(text_bits)
                next_line = f"{name}\t{noise_var}\t{new_weight}\t{score}\n"
